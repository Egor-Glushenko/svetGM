...
class DemoOzonAPI {
  ...
  updateCatalog(newProducts) {
    ...
    // Добавляем в соответствующие секции по категориям
    ...
    newProducts.forEach(product => {
      const containerId = categoryContainers[product.category];
      if (containerId) {
      ...
      <article class="product" data-id="${product.id}" data-category="${product.category}" data-source="ozon" tabindex="0" role="button" aria-label="Подробнее о товаре ${product.title}">
        ...
        <div class="p-body">
          ...
          <div style="display: flex; gap: 8px; margin-top: 8px;">
            <button class="btn btn-outline" data-open="${product.id}" style="flex: 1;">Подробнее</button>
            <button class="btn btn-primary" data-add-cart="${product.id}" style="flex: 1;">В корзину</button>
          </div>
        </div>
      </article>`;
  }
